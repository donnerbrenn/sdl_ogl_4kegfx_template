uniform int runtime;
vec2 v2Resolution = vec2(1920, 1080);
vec3 color;
...
{
  float T = runtime*.001;
  vec2 uv = (2 * gl_FragCoord.xy - v2Resolution) / v2Resolution.y + vec2(1. + 2 * cos(T / 2), 2 * sin(T / 10)) * .5 *rotate(T / 10);
  for (int i = 0; i < 5; i ++) 
  {
    float z = fract((i*.2) + T / 10);
    ...
    color += mix(vec3(0, 0, 1), vec3(1, .4, 0), mix(0, 1, z)) * (1. - smoothstep( .3 * hash21(layeriv),  .3 * hash21(layeriv) + .05, length(layergv - layerr))) * (1. / dot((layerr - layergv) * 25, (layerr - layergv) * 25)) *  (.7 * hash21(layeriv)) * fade + mix(.8 * sin(T / 5) * vec3(.5, .2, 0), vec3(0), length(.5 + uv / 2));
  }
