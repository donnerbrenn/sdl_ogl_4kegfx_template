...
void generateRay( out vec3 rayDir, out vec3 rayPos, in vec2 p, float fiTime )
{
    vec2 s = p;
    ...
    rayPos = rayTar + vec3( -sin(6.2831853*fiTime/20.0), 0.75*cos(6.2831853*fiTime/20.0+0.5), -cos(6.2831853*fiTime/20.0) );

    rayTar += 0.075*vec3( noise3f( vec3(2.0*fiTime,0.0,0.5),0), noise3f( vec3(2.0*fiTime,0.1,0.4),7), noise3f( vec3(2.0*fiTime,0.2,0.3),9) );

    float roll = 0.1*noise3f( vec3(2.0*fiTime,0.0,0.0), 13 );
