...
const char *shader_frag =
 ...
 "{"
   ...
   "v.x*=m.x/m.y;"
   "vec4 f=vec4(0.);"
   "vec3 y=vec3(sin(c)*.5,cos(c*.5)*.25+.25,c),i=normalize(vec3(v.x*1.6,v.y,1.)),l=y,z;"
   "float e=0.;"
   "for(int g=0;g<64;g++)"
     "e=r(l),l+=e*i;"
   "z=l;"
   ...
   "for(int o=0;o<32;o++)"
     "e=r(l),l+=e*i;"
   "f=max(dot(x(l),vec3(.1,.1,0.)),0.)+vec4(.3,cos(c*.5)*.5+.5,sin(c*.5)*.5+.5,1.)*min(length(l-y)*.04,1.);"
   "if(s(z)>n(z))"
